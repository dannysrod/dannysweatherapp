...
export class AppComponent implements OnInit {
  ...
  api = '99cefb99c948d2c1072dd792abe1ae92';
  units = 'metric';
  city1name = 'Sydney';
  city2name = 'Melbourne, Au';
  city3name = 'New York';
  city4name = 'Johannesburg';
  city1;
  city2;
  city3;
  city4;
  ...
  getWeather() {
    ...
    ajax1.open('GET', 'https://api.openweathermap.org/data/2.5/weather?q=' + this.city1name + '&units=' + this.units + '&appid=' + this.api, false);
    ajax1.send();
    this.city1 = JSON.parse(ajax1.responseText);

    const ajax2 = new XMLHttpRequest();
    ajax2.open('GET', 'https://api.openweathermap.org/data/2.5/weather?q=' + this.city2name + '&units=' + this.units + '&appid=' + this.api, false);
    ajax2.send();
    this.city2 = JSON.parse(ajax2.responseText);

    const ajax3 = new XMLHttpRequest();
    ajax3.open('GET', 'https://api.openweathermap.org/data/2.5/weather?q=' + this.city3name + '&units=' + this.units + '&appid=' + this.api, false);
    ajax3.send();
    this.city3 = JSON.parse(ajax3.responseText);

    const ajax4 = new XMLHttpRequest();
    ajax4.open('GET', 'https://api.openweathermap.org/data/2.5/weather?q=' + this.city4name + '&units=' + this.units + '&appid=' + this.api, false);
    ajax4.send();
